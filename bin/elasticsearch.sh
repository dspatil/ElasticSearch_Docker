#!/bin/bash 
# DESC: Elastic_search startup file.
# 
...
ES_CFG_FILE="/opt/elasticsearch-1.7.0/config/elasticsearch.yml"
...
if [[ "$1" == "-"* || -z $1 ]]; then
  /opt/elasticsearch-1.7.0/bin/elasticsearch --config=${ES_CFG_FILE} "$@"
else
  exec "$@"
fi